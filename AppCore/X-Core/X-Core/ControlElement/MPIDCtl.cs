...
namespace X_Core.ControlElement
{
    public partial class MPIDCtl : UserControl
    {
        ...
        public enum eTypes { Void, String, Boolean, Int32, UInt32, Int64, UInt64 };
        ...
        public void BindTwoWay(Expression<Func<string>> propertyLambda)
        {
            ...
            if (_objTarget is CompBase)
            {
                ...
                X_Core.X_CoreS.RegisterOnChanged(propertyLambda, OnChanged);
            }
        ...
        public void UnBind()
        {
            ...
            {
                X_Core.X_CoreS.UnRegisterOnChanged(_propertyLambda, OnChanged);
            }
        ...
        private void OnChanged(string id)
        {
            ...
            if (X_Core.X_CoreS.ComponentExists(ScopeID))
            {
                _rootNode = X_Core.X_CoreS.GetComponent(ScopeID);
            }
            else
            {
                _rootNode = X_Core.X_CoreS.RootComp;
            }
        ...
        private void StoreID()
        {
            ...
            if (_propertySetter != null)
            {
                ...
                if (oldVal != id)
                {
                    ...
                    {
                        X_Core.X_CoreS.LogChange(_compObj.Nickname, oldVal, id);
                    }
                }
            }
            _editing = false;
        ...
        private void BuildAllSections()
        {
            ...
            string id = SplitMethodOrProperty(_primaryStatement, _id);
            BuildMethodArgs(id);
            ...
            if (_argStatements != null)
            {
                ...
                {
                    X_Core.X_CoreS.LogPopup("Unable to find matching method for '{0}'", _id);
                }
            }
        }
        ...
        public string SplitLiteral(IDPropertyList propList, string id)
        {
            ...
            if (tyEnd > 0)
            {
                ...
                if (tgtType == null)
                {
                    try
                    {
                        tgtType = X_Core.X_CoreS.TryGetType(sType);
                    }
                    catch  { }
                }
                if (tgtType == null)
                {
                    X_Core.X_CoreS.LogPopup("expected to find a type declaration for '{0}'", id);
                    tgtType = typeof(string);
                }
            }
            else
            {
                X_Core.X_CoreS.LogPopup("expected to find a closing ')' in type declaration for '{0}'", id);
            }
    ...
    public class IDListBase
    {
            ...
            if (_list.Count > 0)
            {
                ...
                if (index < 0)
                {
                    foreach (IDNodeBase nd in _list)
                    {
                        ...
                        if (index < 0)
                        {
                            X_Core.X_CoreS.LogPopup("Node not found '{0}'.", nd.Text);
                        }
                    }
                }
                if (index > 0)
        ...
        public void SetComp(IDPath nodeStop)
        {
            if (!string.IsNullOrEmpty(nodeStop.Text))
            {
                ...
                if (X_Core.X_CoreS.ComponentExists(id))
                {
                    nodeStop.Comp = X_Core.X_CoreS.GetComponent(id);
                }
            }
        }
        ...
        public IDNodeBase NewSelection(IDNodeBase node, string newText)
        {
            ...
            if (index >= 0)
            {
                ...
                if (newText[index] == '(')
                {
                    ...
                    {
                        X_Core.X_CoreS.LogPopup("Expected method to be in IDMethodList '{0}'", newText);
                    }
    ...
    public class IDPropertyList : IDListBase
    {
        ...
        public override void SetLast(string text)
        {
            ...
            PropertyInfo pi = X_Core.X_CoreS.GetPropertyInfo(_mpIDCtl.BuildFullID(BuildID()));
            if (pi != null)
            {
            ...
            {
                X_Core.X_CoreS.LogPopup("Could not get Property info for '{0}'", text);
            }
        }
        ...
        public void SetLiteralValue(string text, Type tgtType)
        {
            ...
            {
                text = X_Core.X_CoreS.GetDefaultText(tgtType);
            }
            _list.Last().Text = text;
            IDProperty idProp = (_list.Last() as IDProperty);
            idProp.type = _tgtType;
    ...
    public class IDNodeBase
    {
        ...
        {
            // Add Methods
            string[] methodList = X_Core.X_CoreS.GetMethodList(ParentComp, typeof(StateMachineEnabled), _listBase.TgtType);
        ...
        public override string StoreText
        {
            get
            {
                if (IsLiteral)
                {
                    if (string.IsNullOrEmpty(Text))
                    {
                        Text = X_Core.X_CoreS.GetDefaultText(type);
                    }
