...
{
    public partial class CtrBrowser : UserControl
    {
        ...
        public void Rebuild()
        {
            ...
            try
            {
                _rootComp = X_CoreS.RootComp;
                treeComponents.BeginUpdate();
                BuildTree(treeComponents.Nodes, X_CoreS.RootComp);
                treeComponents.EndUpdate();
            }
            catch (Exception ex)
            {
                X_CoreS.LogPopup(ex, "Could not rebuild component tree.");    
            }
        ...
        private void OnChangedCompName(CompBase comp, string oldID)
        {
            ...
            try
            {
                ...
                if (matches.Length > 1)
                {
                    throw new X_CoreException(LogSeverity.Popup, "Found more than one components with same ID: '{0}'", oldID);
                }
                ...
                if (nodeParent == null)
                {
                    throw new X_CoreException(LogSeverity.Popup, "Expected to find a parent for '{0}'", oldID);
                }
            ...
            {
                X_CoreS.LogPopup(ex, "Unable to update Component Tree at: '{0}'", oldID);
            }
            finally
            {
                treeComponents.EndUpdate();
        ...
        private void OnSortedComponentChildren(CompBase compParent)
        {
            ...
            try
            {
                ...
                {
                    X_CoreS.LogPopup("number of component children of '{0}' does not match number of Component Browser children", compParent.Nickname);
                    return;
                }
                if (treeCount == 0)
                {
            ...
            {
                X_CoreS.LogPopup(ex, "Unable to Sort Component Tree at: '{0}'", compParent.ID);
            }
        }
        ...
        private void DoRemovingComponent(string id)
        {
            ...
            try
            {
                ...
                if (nodeParent == null)
                {
                    throw new X_CoreException(LogSeverity.Popup, "Expected to find a parent for '{0}'", id);
                }
                int index = nodeMatch.Index;
                nodeParent.Nodes.Remove(nodeMatch);
            }
            ...
            {
                X_CoreS.LogPopup(ex, "Unable to delete Component Tree at: '{0}'", id);
            }
        }
        ...
        private void DoAddedComponent(CompBase compChild)
        {
            ...
            try
            {
                ...
                if(matches.Length == 0 && compChild.Parent != X_CoreS.RootComp )
                {
                    return;
                }
                ...
                if (matches.Length != 1)
                {
                    throw new X_CoreException(LogSeverity.Popup, "Expected to find a parent for '{0}'", compChild.ID);
                }
                TreeNode nodeParent = matches[0];
            ...
            {
                X_CoreS.LogPopup(ex, "Unable to add Component Tree at: '{0}'", compChild.ID);
            }
        }
        ...
        private void OnNewSelection(object sender, TreeViewEventArgs e)
        {
            ...
            {
                X_CoreS.LogPopup(ex, "Problem with selection");
                return;
            }
