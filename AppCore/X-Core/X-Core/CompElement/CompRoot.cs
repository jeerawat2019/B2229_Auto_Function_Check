...
namespace X_Core.CompElement
{
    public class CompRoot : CompBase
    {
        ...
        public void Add(CompFactory compDef)
        {
            ...
            List<CompFactory> unresolvedPlugins = new List<CompFactory>();
        ...
        public static CompBase ImportFile(Type ty, string filePath)
        {
            ...
            return X_CoreXmlSerializer.LoadObjectFromFile(filePath, ty) as CompBase;
        }
        ...
        private CompBase ReadXmlFile(Type ty, string name)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    X_CoreS.LogPopup(ex, "Could not read '{0}'", path); ;
                }
        ...
        private static void DefineXmlSerializer(Type ty)
        {
            ...
            X_CoreXmlSerializer.DefineSerializer(ty, attrOverrides);
        }
...
", baseType.Namespace, name, baseType.Name, sim);
        ...
        private void CreateSimClasses(CompFactory compDef, List<CompFactory> unresolvedPlugins)
        {
            ...
            //
            List<string> listSources = new List<string>();
            foreach (CompFactory def in unresolvedPlugins)
            {
        ...
        private Type[] CompileClasses(string assemblyName, List<string> listSources)
        {
            ...
            {
                throw new X_CoreExceptionPopup("Unable to compile Plugin Sims. First error:\n{0}", crTemp.Errors[0].ErrorText);
            }
        ...
        public class ThreadTest
        {
            ~ThreadTest()
            {
                X_CoreS.AddThread("GC Finalizer");
            }
        }       /// <summary>
        ...
        public void ApplicationSetup(Form mainForm, string rootFolder)
        {
            ...
            X_CoreS.DumpProcessUsage("AppSetup1");  // Dump Processes running
            ...
            X_CoreS.LogInfo("Application Loading : {0}", _titleText);

            AppStatus("Application Setup");


            X_CoreS.DumpProcessUsage("AppSetup2");  // Dump Processes running
            ...
            X_CoreS.s_mainThreadId = Thread.CurrentThread.ManagedThreadId;
            ...
            X_CoreS.EnsureDirectory(_rootFolder);
            X_CoreS.EnsureDirectory(PluginFolder);
            foreach (string dll in Directory.GetFiles(PluginFolder, "*.dll"))
            {
                ...
                try
                {
                    ...
                    controlTypes.AddRange(assemblyTypes.Where(t => t.IsClass && t.IsSubclassOf(typeof(Control))));
                    X_CoreS.LogInfo("PlugIn loaded: {0}", assemblyPlugIn.FullName);
                }
                catch (Exception ex)
                {
                    X_CoreS.LogPopup("PlugIn Failed: {0}  Reason: {1}", assemblyPlugIn.FullName, ex.Message);
                }
            }
            ...
            X_CoreS.LogInfo("StopWatch Resolution={0}", Stopwatch.IsHighResolution);
            X_CoreS.LogInfo("StopWatch Frequency={0}", Stopwatch.Frequency);

            X_CoreS.DumpProcessUsage("End AppSetup");  // Dump Processes running
        ...
        public static void ExportSettings(string filePath, CompBase compChild)
        {
            try
            {
                X_CoreS.EnsureWritable(filePath);
                DefineXmlSerializer(compChild.GetType());
                X_CoreXmlSerializer.SaveObjectToFile(filePath, compChild);
            }
            catch (Exception ex)
            {
                X_CoreS.LogPopup(ex, "Error Saving '{0}'", compChild.Name);
            }
        }
        ...
        public void SaveSettings() 
        {
            foreach (CompBase compChild in ChildArray)
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    X_CoreS.EnsureDirectory(backupPath);
        ...
        static void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
        {
            ...
            if (o is X_CoreException)
            {
                // Caller knows what to do.  Info is packed in the exception
                X_CoreException mex = o as X_CoreException;
                X_CoreS.Log(mex);
            }
            else if (o is Exception)
            {
                ...
                X_CoreS.LogPopup(ex, "Unhandled Exception");
            }
            else
            {
                X_CoreS.LogFatal("Unhandled Exception {0}", o.ToString());
                Application.Exit();
            }
        }
        static void ApplicationUI_ThreadException(object sender, System.Threading.ThreadExceptionEventArgs e)
        {
            if (e.Exception != null)
            {
                X_CoreS.LogFatal(e.Exception, "Unhandled Exception");
            }
        }
