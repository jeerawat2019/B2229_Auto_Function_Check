...
namespace X_Core.CompElement
{
    public class PropertyWrapper
    {
        ...
        public void Initialize()
        {
            ...
            string propertyName = X_CoreS.SplitPropertyID(ID, out propPath);
            _getter = null;
            if (X_CoreS.ComponentExists(propPath))
            {
                _target = X_CoreS.GetComponent(propPath);
                _pi = X_CoreS.GetPropertyInfo(_target, propertyName, _type);
                if (_pi != null)
                {
            ...
            else
            {
                ...
                if (split.Length == 2)
                {
                    ...
                    if (_type == null)
                    {
                        _type = X_CoreS.TryGetType(split[0]);
                    }
        ...
        public static PropertyWrapper Create(string id, Type expectedType, MethodInvoker delOnChanged)
        {
            ...
            {
                X_CoreS.LogPopup(ex, "Unable to create Property Wrapper");
            }
            return null;
        }
        /// <summary>
