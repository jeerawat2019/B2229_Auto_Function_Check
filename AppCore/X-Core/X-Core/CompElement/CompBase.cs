...
namespace X_Core.CompElement
{
    public class CompBase : INotifyPropertyChanged, IDisposable, IConvertible, IComparable<CompBase>
    {
        ...
        public virtual string Name
        {
            ...
            set
            {
                ...
                catch (X_CoreException mex)
                {
                    X_CoreS.Log(mex);
                }
                catch (Exception ex)
                {
                    X_CoreS.LogPopup(ex, "Unable to change the name to '{0}'.", value);
                }
            }
        }
        /// <summary>
        ...
        public string Nickname
        {
            get
            {
                string[] names = X_CoreS.RootComp.GetUniqueNames(this);
                if (names.Length > 0)
                {
        ...
        public string ID
        {
            get
            {
                if (Parent != null)
                {
                    List<string> names = X_CoreS.RootComp.GetNameList(this);
                    if (names.Count > 0)
                    {
        ...
        public string[] UniqueNames
        {
            get
            {
                return X_CoreS.RootComp.GetUniqueNames(this);
            }
        }
        ...
        private string ValidateName(CompBase parent, string newName)
        {
            if (!(string.IsNullOrEmpty(newName)) && newName.IndexOfAny(new char[] { ',', ';', '[', ']', '.', '+', '-' }) >= 0)
            {
                throw new X_CoreExceptionPopup("Component name '{0}' should not contain any of the following characters: ',;[].+-", newName);
            }
            if (parent != null)
            {
        ...
        {
            X_CoreS.RootComp.UnregisterComponent(this);
        }
        /// <summary>
        ...
        public T GetParent<T>()
        {
            ...
            while (comp != null)
            {
                if (X_CoreS.IsDerivedFrom(comp.GetType(), typeof(T)))
                {
        ...
        public virtual void DumpTiming(StreamWriter writer, string prefix, ref long prevTicks)
        {
            ...
            if (timingOps != null)
            {
                ...
                foreach (TimingElement timingOp in timingOps)
                {
                    ...
                    if (prevTicks != 0)
                    {
                        deltaMS = X_CoreS.TicksToMS(timingOp.ts - prevTicks);
                    }
                    duration += deltaMS;
                    prevTicks = timingOp.ts;
                    DateTime dateTime = new DateTime(timingOp.ts);
                    double msFromMidnight = X_CoreS.TicksToMS(timingOp.ts - midNight.Ticks);
        ...
        {
            X_CoreS.RootComp.RegisterComponent(this);
        }
                ...
                foreach (CompBase comp in arrComp)
                {
                    ...
                    if (comp.Simulate != eSimulate.SimulateDontAsk && plugInClassName.EndsWith(Sim))
                    {
                        ...
                        if (tyPlugIn != null)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    compPlugIn.Destroy();
                                    X_CoreS.LogPopup(ex, "Attempt to use '{0}' plug-in unsuccessful", plugInClassName);
                                    comp.Simulate = eSimulate.SimulateAsk;
                                }
                            }
                            catch (Exception ex)
                            {
                                X_CoreS.LogPopup(ex, "Unexpected error '{0}' plug-in unsuccessful", tyPlugIn.Name);
                                comp.Simulate = eSimulate.SimulateAsk;
                            }
                        }
                        else
                        {
                            ...
                            X_CoreS.LogPopup("'{0}' plug-in is still unsuccessful", plugInClassName);
                    ...
                    catch (Exception ex)  // Also catches ForceSimulateException
                    {
                        ...
                        if (tyPlugInSim != null)
                        {
                            try
                            {
                                X_CoreS.LogPopup(ex, "{0} Initialize failed but it will simulate.", plugInClassName);
                            ...
                            {
                                X_CoreS.LogPopup(exc, "{0} Initialize failed and is unable to simulate.", comp.Nickname);
                            }
                        }
                        else
                        {
                            X_CoreS.LogPopup(ex, "{0} Initialize failed and is unable to simulate.", comp.Nickname);
                        }
                    }
                }
            }
        ...
        {
            X_CoreS.EnsureDirectory(Path.GetDirectoryName(filePath));
            X_CoreXmlSerializer.SaveObjectToFile(filePath, this);
        }
        ...
        public virtual void AppendAt(CompBase anchorChild, CompBase newChild)
        {
            ...
            {
                X_CoreS.LogPopup(ex, "Unable to append '{0}'", newChild.Name);
            }
        }
        ...
        public virtual void Add(CompBase newChild)
        {
            ...
            {
                X_CoreS.LogPopup(ex, "Unable to add '{0}'", newChild.Name);
            }
        }
        ...
        public void SortChildren()
        {
            ...
            {
                X_CoreS.LogPopup(ex, "Error in ordering children for {0}", ID);
            }
            finally
            {
                __listChildrenLock.ExitWriteLock();
        ...
        {
            if (X_CoreS.IsDerivedFrom(this.GetType(), conversionType))
            {
                return this;
            }
            return null;
        ...
        public T GetPropValue<T>(Expression<Func<T>> property)
        {
            ...
            if (propItem == null)
            {
                T val = (T)X_CoreS.GetDefault(typeof(T));
                if (val == null)
                    val = default(T);
                return val;
            }
        ...
        private PropertyItem<T> GetPropertyItem<T>(Expression<Func<T>> property, out bool firstTime)
        {
            ...
            {
                X_CoreS.LogPopup(ex, "Problem reading the property item '{0}'", propName);
                return null;
            }
            finally
            {
            ...
            {
                X_CoreS.LogPopup(ex, "Problem creating the property item '{0}'", propName);
            }
            finally
            {
        ...
        private void RaisePropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            Control c = X_CoreS.GetDummyControl();
            if (c == null)
            {
        ...
        public class NotifyItem<T> : NotifyItemBase<T>
        {
            ...
            public override void Fire(T value)
            {
                ...
                catch (Exception ex)
                {
                    if (_del.Target is Control)
                    {
                        X_CoreS.LogPopup(ex, "Trouble with FireToControl for control '{0}'", (_del.Target as Control).Name);
                    }
                    else
                    {
                        X_CoreS.LogPopup(ex, "Trouble with FireDelegate of delegate {0}", _del.ToString());
                    }
        ...
        public class NotifyItemSender<S, T> : NotifyItemBase<T>
        {
            ...
            public override void Fire(T value)
            {
                ...
                catch (Exception ex)
                {
                    if (_del.Target is Control)
                    {
                        X_CoreS.LogPopup(ex, "Trouble with FireToControl of '{0}' for control '{1}'", _sender.ToString(), (_del.Target as Control).Name);
                    }
                    else
                    {
                        X_CoreS.LogPopup(ex, "Trouble with FireDelegate of '{0}' delegate {1}", _sender.ToString(), _del.ToString());
                    }
                }
            }
        }
        ...
        public class PropertyItem<T> : PropertyItemBase
        {
            ...
            public bool IsValueDifferent(T newVal)
            {
                ...
                {
                    X_CoreS.LogPopup(ex, "Trouble with Equal comparison for {0} type", typeof(T).Name);
                }
                finally
                {
            ...
            public T Value
            {
                ...
                set
                {
                    ...
                    {
                        X_CoreS.LogPopup(ex, "Trouble with value assignment for '{0}' of type {1}", _propName, typeof(T).Name);
                    }
                    finally
                    {
            ...
            public void Add(NotifyItem<T>.DelPropNotify del)
            {
                ...
                {
                    X_CoreS.LogPopup(ex, "Could not add a callback ({0}) to '{1}'", del.ToString(), _propName);
                }
            }
            /// <summary>
            /// Add a callback to this property
        ...
        {
            Type[] classes = X_CoreS.GetClassHierarchy(GetType(), typeof(CompBase));
                ...
                try
                {
                    ...
                    foreach (PropertyInfo pi in props)
                    {
                        try
                        {
                            ...
                            if (oVal is X_UnitBase)
                            {
                                strLog += (oVal as X_UnitBase).Val.ToString("##0.###");
                                strHeader += string.Format("{0} ({1})", pi.Name, (oVal as X_UnitBase).UnitText);
                            }
                            else
                            {
                                if (oVal == null)
            ...
            string filepath = X_CoreS.GetLogFilePath(filename);
            if (!File.Exists(filepath))
            {
                X_CoreS.LogCustom(filename, strHeader);
            }

            X_CoreS.LogCustom(filename, strLog);
        }
        ...
        public virtual int CompareTo(CompBase otherComp)
        {
            ...
            for (int i = 0; i < split.Length; i++)
            {
                ...
                if (X_CoreS.IsNumber(split[i]) && X_CoreS.IsNumber(splitOther[i]))
                {
    ...
    {
        public long ts = X_CoreS.DateTimeNow;
