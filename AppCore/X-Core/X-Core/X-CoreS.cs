...
{
    /// <summary>
    /// Static helper functions intended to enhance X-Core
    /// </summary>
    public static class X_CoreS
    {
        ...
        private static long _lastProcessDump = X_CoreS.DateTimeNow;
        ...
        {
            long enterTick = X_CoreS.DateTimeNow;
            double msSinceLast = X_CoreS.TicksToMS(enterTick - _lastProcessDump);
            X_CoreS.LogInfo("Dumping Processes at {0}.  Current total CPU Usage=,{1}, time since last=,{2}", tag, RootComp.CPUPerc, msSinceLast.ToString("####0.#"));
            ...
            long time1 = X_CoreS.DateTimeNow;
            double dElapsed = X_CoreS.TicksToMS(time1 - enterTick);
            X_CoreS.LogInfo("Time to getProceses = {0} mS", dElapsed.ToString("###,###.##"));
            ...
            foreach (ServiceController service in services)
            {
                ...
                if (service.Status == ServiceControllerStatus.Running && !_svcExceptions.Contains(service.ServiceName))
                {
                    ...
                    catch
                    {
                        ...
                        X_CoreS.LogInfo("** Service threw exception: '{0}'", service.DisplayName);
                    }
                }
            }
            dElapsed = X_CoreS.TicksToMS(X_CoreS.DateTimeNow - time1);
            X_CoreS.LogInfo("Time to extract services = {0} mS", dElapsed.ToString("###,###.##"));
            ...
            for (int i = _prevProcesslist.Count-1; i >= 0; i--)
            {
                ...
                {
                    // Not there anymore
                    X_CoreS.LogInfo("Removed Id={0}, Name={1}", keyVal.Key, keyVal.Value.procName);
                ...
                else
                {
                    ...
                    {
                        X_CoreS.LogInfo("{0} Id={1}, Name={2}, {3},(%),{4},(Sec)", keyVal.Value.type, keyVal.Key, keyVal.Value.procName, perc.ToString("###0.##"), secSinceLast.ToString("###0.##"));
                        if (servicesByPid.ContainsKey(keyVal.Key))
                        {
                            foreach (string serviceName in servicesByPid[keyVal.Key])
                            {
                                X_CoreS.LogInfo("** Service'{0}'", serviceName);
                            }
                        }
                        else if (keyVal.Key == curProcess.Id)
                        {
                            ...
                            foreach (ProcessThread procThread in threads)
                            {
                                try
                                {
                                    ...
                                    if (ms > 0.1)
                                    {
                                        X_CoreS.LogInfo("** Thread {0}({1}) = {2} mS", name, procThread.Id, ms.ToString("###,##0.#"));
                                    }
                                }
                                catch { }
                            }
            ...
            dElapsed = X_CoreS.TicksToMS(X_CoreS.DateTimeNow - enterTick);
            X_CoreS.LogInfo("Total time to dump = {0} mS", dElapsed.ToString("###,###.##"));
        ...
        public static int GetChecksum(Bitmap bm)
        {
            ...
            {
                X_CoreS.LogPopup(ex, "Problem finding the checksum for Bitmap");
            }
            return 0;
        }
        ...
        public static object GetDefault(Type type)
        {
            ...
            {
                X_CoreS.LogPopup(ex, "Could not get default for type '{0}'", type.Name); 
            }
            return null;
        }
        ...
        public static void InvokeNoArgMethod(string methodID)
        {
            ...
            if (lastPeriod < 0)
            {
                X_CoreS.LogPopup("Expected a target in string: '{0}'", methodID);
                return;
            }
            string compID = methodID.Substring(0, lastPeriod);
            if (!ComponentExists(compID))
            {
                X_CoreS.LogPopup("Not a valid component ID ('{0}') in ID string: '{1}'", compID, methodID);
                return;
            }
            CompBase target = X_CoreS.GetComponent(compID);
            ...
            if (mi == null)
            {
                X_CoreS.LogPopup(exGetMethod, "Could not find a method named '{0}.{1}' in ID string: '{2}'", methodName, compID, methodID);
                return;            
            }
            try
            {
            ...
            {
                X_CoreS.LogPopup(ex, "Could not invoke the call '{0}'", methodID);
            }
        ...
        public static Bitmap RotateImage(Bitmap imageSource, double degrees)
        {
            ...
            {
                X_CoreS.LogError(ex, "Error to RotateImage");
            }
            return bmRet;
        }
        ...
        public static double Formulae(string formulae, params string[] assignments)
        {
            ...
            if (assignments != null)
            {
                foreach (string assignment in assignments)
                {
                    ...
                    {
                        X_CoreS.LogPopup("Unexpected formulae assignment {0}", assignment);
                        return 0.0;
                    }
                }
            }
        ...
        public static void DeleteLogFile(string filename)
        {
            ...
            {
                X_CoreS.LogPopup(ex, "Unable to delete log file '{0}'", filename);
            }
        }

        public static string GetLogFilePath(string filename)
        {
            return string.Format(@"{0}\Log\{1}", X_CoreS.RootComp.RootFolder, filename);
        }
        ...
        public static void LogCustom(string filename, string msg, string header)
        {
            ...
            X_CoreS.EnsureDirectory(strFullLogFileName);
        ...
        {
            if (ex is X_CoreException)
            {
                X_CoreException mex = ex as X_CoreException;
       ...
       /// <summary>
        ...
        /// <code>
        ///    X_CoreS.WriteLog(X_CoreS.LogSeverity.Error, "Testing", "Dump Log File",
        ///               X_CoreS.LogAttachment.ScreenShot | X_CoreS.LogAttachment.ProcessDump,
