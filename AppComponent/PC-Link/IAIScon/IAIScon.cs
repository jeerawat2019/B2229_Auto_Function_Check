...
namespace MCore.Comp.MotionSystem
{
    ...
    public class IAIScon:MotionSystemBase
    {
        ...
        private bool _dateReceive = false;
        private int _retrySendMessage = 0;
        private const int _maxretrySendMessage = 3;
        ...
        {
            get { return GetPropValue(() => ReadTimeOut, 3000); }
            set { SetPropValue(() => ReadTimeOut, value); }

        }

        [Category("Motion TimeOut")]
        public Miliseconds WaitMoveTimeOut
        {
            get { return GetPropValue(() => WaitMoveTimeOut, 2000); }
            set { SetPropValue(() => WaitMoveTimeOut, value); }

        }

        [Category("Motion TimeOut")]
        public Miliseconds WaitHomeTimeOut
        {
            get { return GetPropValue(() => WaitHomeTimeOut, 2000); }
            set { SetPropValue(() => WaitHomeTimeOut, value); }
        ...
        public IAIScon(string name, int  com)
            : base(name)
        {
            // Only used to create 1st xml data file
            PortSetting.CommPort = (IAIRS232Settings.eComport)com;

        }

        public IAIScon(string name, int com, Miliseconds readTimeOut)
        {
            ReadTimeOut = readTimeOut;
            PortSetting.CommPort = (IAIRS232Settings.eComport)com;
        }

        public IAIScon(string name, IAIRS232Settings  _portSetting)
            : base(name)
        {
            // Only used to create 1st xml data file
        ...
        public override void HomeAxis(RealAxis axis)
        {
            ...
            string tmp = axisNumber.ToString("X2");
            int HexAxisNo = Convert.ToInt32(tmp, 16);
            ...
            try
            {
                lock (this)
                {
                    ...
                    //_waitDataRecived.Reset();
                    //_modbusClient.WriteSingleCoil(Cmd, false);
                    
                    //WaitDataReceived(ReadTimeOut);

                    //_waitDataRecived.Reset();
                    //_modbusClient.WriteSingleCoil(Cmd, true);
                    //WaitDataReceived(ReadTimeOut);
            ...
            WaitForHomeDone(50000);
            //WaitForHomeDone(WaitHomeTimeOut.ToInt);
            axis.Homed = true;
            System.Threading.Thread.Sleep(100);
            lock (this)
            {
        ...
        {
            //base.MoveRelAxis(axis, speed, waitForCompletion);
             try
                {

                   // int iPos = (int) ((axis.TargetPosition + axis.CurrentPosition )/ 0.01) ;
                   // int iPos = (int)(axis.CurrentPosition / 0.01);
                    int iPos = (int)(axis.TargetPosition / 0.01);
                    int iInpos = (int)(1 * 10);
                    int iSpeed = (int)(speed / 0.01);
                    int iAccel = (int)(axis.AccelDecel / 0.01);
                    //int iAccel = (int)(0.1 / 0.01);
                    ...
                    //for (_retrySendMessage = 0; _retrySendMessage < _maxretrySendMessage; _retrySendMessage++)
                    //{
                        lock (this)
                        {
                            _waitDataRecived.Reset();
                            ...
                            WaitDataReceived(ReadTimeOut);
                            //byte[] rec = WaitDataReceived(ReadTimeOut);
                            //if (rec != null)
                            //    break;
                        }
                    //}
                        ...
                        //WaitForMoveDone(WaitMoveTimeOut.ToInt);
             ...
             catch (Exception ex)
             {
                 ...
                 throw new MCoreExceptionPopup(ex, "Move Rel Command fail {0}", this.Nickname);
             }
        }
                ...
                try
                {
                    lock (this)
                    {
                        ...
                        if (timeoutStopw.ElapsedMilliseconds > timeout)
                        {
                            ...
                            //if (_retrySendMessage >= _maxretrySendMessage)
