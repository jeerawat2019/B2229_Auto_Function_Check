...
namespace X_Core.Comp.SMLib.SMFlowChart.EditForms
{
    public partial class ActionEditorForm : Form
    {
        ...
        private void AddMethod(SMMethod method)
        {
            ...
            // compIDCtl
            // 
            MPIDCtl compIDCtl = new X_Core.ControlElement.MPIDCtl();
            _list.Add(compIDCtl);
            // 1 based
            int count = _list.Count;
            SetNewSizes();
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            for(; i<_list.Count-1; i++)
            {
                if (i < _actionFlowItem.Count)
                {
                    ...
                    if (oldVal != _list[i].ID)
                    {
                        ...
                        X_CoreS.LogChange(string.Format("{0}.Method-{1}", _actionFlowItem.Nickname, i), oldVal, _list[i].ID);
                    }
                }
                else
                {
                    ...
                    X_CoreS.LogChangeAdded(string.Format("{0}.Method-{1} : {2}", _actionFlowItem.Nickname, i, method.MethodID));
                }
            }
            ...
            for (; i < count; i++)
            {
                ...
                X_CoreS.LogChangeRemoved(string.Format("{0}.Method-{1} : {2}", _actionFlowItem.Nickname, iRemove, (_actionFlowItem[iRemove] as SMMethod).MethodID));
