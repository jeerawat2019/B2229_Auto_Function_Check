...
using X_Core.Comp.SMLib.SMFlowChart;
...
namespace X_Core.Comp.SMLib.SMFlowChart.EditForms
{
    public partial class DecisionEditorForm : Form
    {
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            {
                X_CoreS.LogChange(string.Format("{0}.Label", _decisionItem.Nickname), _decisionItem.Text, tbText.Text);
                _decisionItem.Text = tbText.Text;

            }
            if (_decisionItem.ConditionID != DummyID)
            {
                X_CoreS.LogChange(string.Format("{0}.ID", _decisionItem.Nickname), _decisionItem.ConditionID, DummyID);
