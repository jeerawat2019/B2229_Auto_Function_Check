...
namespace X_Core.Comp.SMLib.Flow
{
    public class SMFlowContainer : SMFlowBase
    {
        ...
        {
            X_CoreS.RegisterOnChanged(() => ScopeID, OnScopeChanged);
            base.Initialize();
            _prevScope = ScopeID;
        }
        ...
        {
            base.Destroy();
            X_CoreS.UnRegisterOnChanged(() => ScopeID, OnScopeChanged);
        }
        #endregion overrides
        ...
        public void FlowJump(string name)
        {
            _currentFlowItem = ChildArray.Single(c => c.Name == name) as SMFlowBase;
        ...
        public void AutoScope(bool autoScope)
        {
            ...
            {
                ScopeID = newScope;
                X_CoreS.LogChange(string.Format("{0}.Scope", Nickname), oldScope, newScope); 
            }
        ...
        public bool RegisterEdit(string ID)
        {
            if (string.IsNullOrEmpty(_editRegisterationID))
            {
                ...
                X_CoreS.LogChange(string.Format("{0}, Start editing", Nickname));
                return true;
            }
            return false;
        }
        ...
        public bool UnregisterEdit(string ID)
        {
            ...
            {
                X_CoreS.LogChange(string.Format("{0}, Finish editing", Nickname));
                        ...
                        if (pathOut != null && !pathOut.HasTargetID && decisionItem.WaitTimeoutMS > 0)
                        {
                            ...
                            else
                            {
                                    ...
                                    (!_currentFlowItem.HasChildren) && _timeoutHandle.WaitOne(0))
                                {
                                    ...
                                    {
                                        X_CoreS.LogWarning("Decision Wait Time Out [{0}] on state [{1}]", decisionItem.PathText, this.StateMachine);
        ...
        {
            if (X_CoreS.GetDummyControl().InvokeRequired)
            {
                X_CoreS.GetDummyControl().BeginInvoke(new _delParamObject(DisplayAsynchMsg), new object[] { param });
                return;
            }
