    ...
    public class SMPathOutBool : SMPathOut
    {
        ...
        public override bool HasTargetID
        {
            get 
            {
                ...
                {
                    if (X_CoreS.DateTimeNow < TimeToEnableTargetID)
                    {
                        return false;
                    }
                    double timeOver = X_CoreS.TicksToMS(X_CoreS.DateTimeNow - TimeToEnableTargetID);
                    if (timeOver < 200)
                    {
                        X_CoreS.LogWarning("Decision ({0}) Timed out at {1} mS", Owner.BestDisplayText, PathOutDelayMS);
                    }
        ...
        public void ApplyPathDelay(SMFlowContainer parentContainer)
        {
            if (TimeToEnableTargetID == 0 && PathOutDelayMS > 0)
            {
                ...
                TimeToEnableTargetID = X_CoreS.DateTimeNow + X_CoreS.MSToTicks(PathOutDelayMS);
            }
        }
