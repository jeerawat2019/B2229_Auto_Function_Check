...
namespace B2229_AT_FuncCheck.Dev_AppStation.Controller
{
    public class PLC_System : CompBase
    {
        ...
        public void GetMemControlToPart()
        {
            ...
            {
                PropertyInfo propertie = x.Value.GetType().GetProperty("MemBinProcess");
                ///
                string memResult = null;
                ///
                //GetMemControlWord(Dev_AppMachine.Machine.This.PartJigColSfit1.MemControlPart.ToString(), out memResult);
                ///
                propertie.SetValue(x.Value, PLCConvert.DecimalToBinary(io[i]));i++;
                ///
                return true;
            });
