...
namespace X_Core
{
    ...
    /// </summary>
    public class X_CoreXmlSerializer
    {
        // Static mapping
        private static Dictionary<Type, X_CoreXmlSerializer> s_serializers = new Dictionary<Type, X_CoreXmlSerializer>();
        ...
        public X_CoreXmlSerializer(Type tyRoot)
        {
        ...
        public X_CoreXmlSerializer(Type tyRoot, XmlAttributeOverrides overrides)
        {
        ...
        public static X_CoreXmlSerializer DefineSerializer(Type ty, XmlAttributeOverrides overrides)
        {
            X_CoreXmlSerializer ser = null;
            ...
            try
            {
                ...
                {
                    ser = new X_CoreXmlSerializer(ty, overrides);
                    s_serializers.Add(ty, ser);
                }
            }
            catch (Exception ex)
            {
                X_CoreS.LogPopup(ex, "Unable to define serializer for type {0}", ty.Name);
            }
            finally
            {
        ...
        /// <returns></returns>
        /// <remarks>We cache it because it takes so long to create the MclXmlSerializer object.</remarks>
        public static X_CoreXmlSerializer DefineSerializer(Type ty)
        {
        ...
        /// <returns></returns>
        /// <remarks>We cache it because it takes so long to create the MclXmlSerializer object.</remarks>
        private static X_CoreXmlSerializer GetSerializer(Type ty)
        {
        ...
        {
            X_CoreXmlSerializer ser = X_CoreXmlSerializer.GetSerializer(ty);
            object obj = null;
        ...
        {
            X_CoreXmlSerializer ser = GetSerializer(obj.GetType());
        ...
        private void Setup(Type tyRoot, XmlAttributeOverrides overrides)
        {
            ...
            catch (X_CoreException)
            {
                throw;
            }
            catch (Exception ex)
            {
                throw new X_CoreExceptionPopup(ex, "Exception");
            }
        }
        /// <summary>
        /// Serialize the object
        ...
        private void SerializeClassObject(XmlDocument xDoc, XmlNode xmlNodeParent, object objClass, ClassField cfArray, string xmlClassElementName)
        {
            ...
            try
            {
                ...
                {
                    X_CoreS.LogPopup("Xml serializer is missing class '{0}'", classTypeFullName);
                    return;
                }
                ...
                try
                {
                    ...
                    if (objValue != null)
                    {
                        ...
                        if (cf.bArray)
                        {
                            ...
                            {
                                throw new X_CoreExceptionPopup("Unsupported array type ({0})", xmlElementName);
                            }
                            
                            if (objValueType.IsGenericType && objValueType.GetGenericArguments().Length != 1)
                            {
                                throw new X_CoreExceptionPopup("Only one-dimensional generic arrays supported ({0})", xmlElementName);
                            }
        ...
        private void BuildClassTypeListRecurse(Type tyClass, XmlAttributeOverrides overrides)
        {
            ...
            {
                throw new X_CoreExceptionPopup(ex, "Unable to add '{0}' to '{1}'", tyClass.FullName, xmlClassName);
            }
            ...
            while (ty != null && ty != typeof(object))
            {
                ...
                {
                    throw new X_CoreExceptionPopup(ex, "Unable to set attrsClassOverrides (type= '{0}'", ty.Name);
                }
                MemberInfo[] miList = ty.GetMembers(BindingFlags.Instance | BindingFlags.GetField | BindingFlags.GetProperty | BindingFlags.DeclaredOnly | BindingFlags.Public);
                ...
                for (int i = miList.Length-1; i >= 0; i--)
                {
                    ...
                    switch (mi.MemberType)
                    {
                        ...
                        case MemberTypes.Field:
                            ...
                            {
                                throw new X_CoreExceptionPopup(ex, "Unable to MapFieldMember (FieldName= '{0}')", mi.Name);
                            }
                            break;
        ...
        private void MapFieldMember(XmlAttributeOverrides overrides,  ClassMapping cm, MemberInfo mi)
        {
                ...
                foreach (object customAttr in customFieldAttrs)
                {
                    ...
                    else if (customAttr is XmlElementAttribute)
                    {
                        ...
                        if (!string.IsNullOrEmpty(xmlElement.ElementName))
                        {
                            ...
                            {
                                throw new X_CoreExceptionPopup(ex,
            ...
            {
                throw new X_CoreExceptionPopup(ex, "Reading Custom Attributes");
            }
            try
            {
                Type ty = cm.ty;
        ...
        private System.Collections.IList InstantiateArray(FieldNameInfo fni, object objParent, XmlNode xmlArrayNode)
        {
            ...
            {
                throw new X_CoreExceptionPopup("fniCurLevelList.curLevelList is null: ({0})", fni.cf.mi.Name);
            }
            int count = 0;
            ...
            if (listType.Name.Contains("[]"))
            {
                ...
                catch (X_CoreExceptionPopup)
                {
                    throw;
                }
                catch (Exception ex)
                {
                    throw new X_CoreExceptionPopup(ex, "Unable create an Array: {0}", listType.FullName);
                }
            }
            else
            {
                ...
                {
                    throw new X_CoreExceptionPopup(ex, "Unable create an IList array: {0}", listType.FullName);
                }
            }
        }
        private object InstantiateAndPopulateClassRecurse(XmlNode xmlNode, Type tyClass)
        {
            ...
            {
                throw new X_CoreExceptionPopup(ex, "Problem with default constructor for '{0}'", tyClass.FullName);
            }
            ...
            for (int n = 0; n < xmlNode.ChildNodes.Count; n++)
            {
                ...
                if (cm.xmlElementNames.ContainsKey(xmlChild.Name))
                {
                    ...
                    else if (HasChildValues(fni.xmlElementType))
                    {
                        ...
                        {
                            throw new X_CoreExceptionPopup("Unknown Deserialize problem with '{0}'", fni.xmlElementType.FullName);
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    X_CoreS.LogWarning("class= {0}  xmlElementName={1}", tyClass.FullName, xmlChild.Name);
                }
            }
        ...
        private object CreateArrayItem(ClassField cf, XmlNode nodeArrayItem)
        {
            ...
            if (am != null)
            {
                ...
                else if (arrayItemType != null && arrayItemType.IsClass)
                {
                    ...
                    X_CoreS.LogPopup("Class not found: {0}", nodeArrayItem.Name);
            ...
            {
                throw new X_CoreExceptionPopup("Could not determine type for array item: {0}", nodeArrayItem.Value);
            }
            ...
            throw new X_CoreExceptionPopup("Could not find the value for array item: {0}", nodeArrayItem.Value);
        }
        /// <summary>
    ...
    class NameInfoBase
    {
        ...
        {
            return X_CoreXmlSerializer.ParseForTypedValue(valString, xmlElementType);
        }
    }
    ...
    class FieldNameInfo : NameInfoBase
    {
        ...
        public void SetXmlValue(object objClass, XmlNode xmlNode)
        {
            ...
            if (xmlNode.HasChildNodes)
            {
                ...
                {
                    X_CoreS.LogPopup("Expected a Text Value node for ({0})", xmlNode.Name);
                }
            }
            else
            {
    ...
    class ArrayMapping
    {
        ...
        public void Add(string name, Type ty)
        {
            ...
            {
                X_CoreS.LogPopup("Duplicate name in arrayNameList: Name={0}  Type={1}", name, ty.Name);
            }
            else
            {
                try
                ...
                {
                    throw new X_CoreExceptionPopup(ex, "Trouble with adding {0}", name);
                }
            }
        }
    }
